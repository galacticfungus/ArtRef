...
// use super::Gpu;
use super::{QueueFamily, PciVendor, Features, Feature, ExtensionManager, DeviceExtensions, QueueManager, ConfigureDevice, DeviceQueue};
use crate::error;
...
impl<'a> ConfigureDevice<'a> {
    pub fn new(instance: &'a erupt::InstanceLoader, 
        ...
        present_modes: Vec<surface::PresentModeKHR>) -> ConfigureDevice<'a> {
        ConfigureDevice {
            instance,
            device_handle,
        ...
        F: Fn(&mut QueueManager) -> (),
    {
        ...
        self.queues_to_create = queues_to_create.into_iter().map(|(index, queue)| queue).collect();
    ...
    pub fn create_device(self) -> VulkanDevice {
        ...
        let device = erupt::DeviceLoader::new(self.instance, self.device_handle, &create_info, None)
            .expect("Failed to create device");
