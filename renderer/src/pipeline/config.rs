...
use super::{ConfigureRasterizer, ConfigureShader, ConfigureShaders};
...
impl<'a> ConfigurePipeline<'a> {
    ...
    pub fn configure_shaders<F>(mut self, define_shaders: F) -> Self
    where
        F: FnOnce(&mut ConfigureShaders) -> (),
    {
...
pub struct ConfigureVertexInput<'a> {
