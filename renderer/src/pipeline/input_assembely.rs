...
impl<'a> ConfigureInputAssembely for ConfigurePipeline<'a> {
    fn configure_input_assembely(
        &mut self, configure_assembely: &mut dyn FnMut(&mut InputAssembelySettings)
...
impl<'a, 'b: 'a> InputAssembelySettings<'a, 'b> {
    pub fn new(pipeline_input: &'a mut vk::PipelineInputAssemblyStateCreateInfoBuilder<'b>) -> InputAssembelySettings<'a, 'b> {
    ...
    pub fn set_restart(&mut self, can_restart: bool) {
        self.pipeline_assembely.primitive_restart_enable(can_restart);
    }
}
