...
impl<'a> ConfigureShaders<'a> {
    ...
    pub fn create_fragment_shader(
        ...
        let shader_data =
            self.prepare_shader(entry_name, code, vk::ShaderStageFlagBits::FRAGMENT)?;
    ...
    pub fn prepare_shader(
    ...
    ) -> Result<ShaderData, Error> {
        let owned_name = CString::new(entry_name).map_err(|err| {
            Error::new(
                ErrorKind::InvalidShaderEntryMethodName(Vec::from(entry_name.as_bytes())),
                None,
            )
        })?;
        ...
        let shader_module = unsafe {
            self.device
                .create_shader_module(&shader_module_builder, None, None)
        }
