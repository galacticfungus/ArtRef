...
use renderer::{ VulkanConfig, PciVendor, PresentMode, SurfaceFormat, SurfaceColourSpace, InstanceExtensions, DeviceExtensions};
...
fn main() {
    ...
    let mut selector = vulkan_api.select_device(&mut surface).expect("Failed to create selector");
    // Filter devices based on requirements
    let gpu = selector.required_device_extensions(|mng| {
    ...
    gpu_config.enable_feature(Features::GeometryShader).expect("Failed to enable feature")
    ...
    configure_swapchain.select_present_mode(|mng|{
    ...
    let swapchain = configure_swapchain.build().expect("Failed to build swapchain");
